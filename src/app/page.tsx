...
export default function HomePage() {
  ...
  function ProductCard({ product, onAddToCart, onToggleWishlist, isWishlisted }: any) {
    return (
      <div className="bg-[#1e293b] rounded-lg p-3 hover:bg-cyan-900 transition-all duration-300 hover:scale-105 flex flex-col h-80 cursor-pointer group relative">
        ...
        <div className="flex-1 flex flex-col justify-between min-h-0">
          ...
          <div className="flex gap-2 justify-center mt-auto pt-4 relative z-20">
            <button
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 mb-8">
          {topSellers.length === 0 ? (
            ...
            topSellers.map((product: any) => (
              <div key={product.id} className="bg-[#1e293b] rounded-lg p-3 hover:bg-cyan-900 transition-all duration-300 hover:scale-105 flex flex-col h-80 cursor-pointer group relative">
                <Link href={`/product/${product.id}`} className="absolute inset-0 z-10" aria-label={`View ${product.name}`} />
                <div className="w-full h-24 bg-gradient-to-br from-cyan-500 to-blue-600 rounded-md mb-3 flex items-center justify-center overflow-hidden flex-shrink-0">
                  {product.images && product.images.length > 0 && product.images[0] ? (
                    <img src={product.images[0]} alt={product.name} className="w-full h-full object-cover" />
